...
func NewDatastore(parentCtx context.Context, pmf *backendmetrics.PodMetricsFactory) Datastore {
	store := &datastore{
		parentCtx:            parentCtx,
		poolAndObjectivessMu: sync.RWMutex{},
...
type datastore struct {
	...
	// poolAndObjectivessMu is used to synchronize access to pool and the objectives map.
	poolAndObjectivessMu sync.RWMutex
...
func (ds *datastore) Clear() {
	ds.poolAndObjectivessMu.Lock()
	defer ds.poolAndObjectivessMu.Unlock()
	ds.pool = nil
...
func (ds *datastore) PoolGet() (*v1.InferencePool, error) {
	ds.poolAndObjectivessMu.RLock()
	defer ds.poolAndObjectivessMu.RUnlock()
	if !ds.PoolHasSynced() {
