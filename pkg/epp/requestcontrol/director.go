...
func (d *Director) HandleRequest(ctx context.Context, reqCtx *handlers.RequestContext) (*handlers.RequestContext, error) {
	...
	if reqCtx.ShortCircuit == 2 {
		pool, err := d.datastore.PoolGet()
		if err != nil {
			return reqCtx, err
		}
		shortCircuitPod := d.GetRandomPod()
		endpoint := net.JoinHostPort(shortCircuitPod.Address, strconv.Itoa(int(pool.Spec.TargetPortNumber)))
		reqCtx.TargetPod = shortCircuitPod
		reqCtx.TargetEndpoint = endpoint
		logger.V(logutil.DEFAULT).Info("Short-circuiting request", "targetPod", reqCtx.TargetPod, "targetEndpoint", reqCtx.TargetEndpoint)
		return reqCtx, nil
	}
